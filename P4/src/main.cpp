...
#define Pulsador2 2 //Pulsador 1
#define LED_ROJO 47  // LED rojo en el pin 47
#define LED_VERDE 39 // LED verde en el pin 39
#define PULSADOR 48  // Pulsador en el pin 46
SemaphoreHandle_t xSemaphore;

void tareaEncenderRojo(void *pvParameters) {
    while (1) {
        if (digitalRead(PULSADOR) == LOW and digitalRead(Pulsador2) == LOW) { // Pulsador presionado (asumiendo pull-up interno)
        ...
        vTaskDelay(pdMS_TO_TICKS(50)); // Pequeña espera para evitar rebotes
    }
}
...
void setup() {
    ...
    pinMode(PULSADOR, INPUT_PULLUP); // Configura el pulsador con resistencia pull-up interna
